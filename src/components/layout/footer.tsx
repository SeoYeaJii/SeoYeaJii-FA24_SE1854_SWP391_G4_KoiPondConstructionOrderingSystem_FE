...
const Footer = () => {
  return (
    <div className="w-full min-h-[336px] phone:min-h-[329px] flex-shrink-0 z-50 bg-[#056839] pt-[5vw]">
      <div className="w-[79.17%] tablet:w-[90%] phone:w-[90%] flex phone:flex-col justify-start items-start gap-[100px] phone:gap-[15px] mx-auto relative">
        ...
        <div className="flex justify-start items-start gap-20 w-[74.5%] phone:w-full">
          {sections.map((section, index) => (
            <div
              className="flex gap-[15px] w-full flex-col phone:hidden"
              key={index}
            >
              <Typography
                variant={"medium"}
                fontWeight={"bold"}
                className="uppercase phone:!text-[16px]"
              >
                {section.title}
              </Typography>
          ...
          <div className="flex gap-[15px] w-full flex-col">
            ...
            <div className="flex flex-col gap-[10px]">
              {contact.items.map((item, index) => (
                <div
                  className="flex gap-[10px] cursor-pointer items-start"
                  key={index}
                >
                  <img
                    src={item.icon}
